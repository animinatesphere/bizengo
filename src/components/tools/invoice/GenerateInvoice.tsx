...
const GenerateInvoice = ({
  // Renamed from InvoiceGenerator to GenerateInvoice to avoid conflict
  invoiceType,
  businessName,
  businessAddress,
  businessPhone,
  ...
  const generateQRCode = async () => {
    if (!customerName || !businessName || invoiceType === "") {
      toast({
        ...
        description:
          "Customer name, business name, or document type is missing. Cannot generate QR code.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      switch (selectedTemplate) {
        ...
        default:
          toast({
            title: "Error",
            description:
              "Selected template not supported for native PDF generation.",
            variant: "destructive",
          });
          return null;
      }
      ...
      const fileName =
        invoiceType === "invoice" ? "invoice.pdf" : "receipt.pdf";
      formData.append("file", pdfBlob, fileName);

      const response = await fetch(
        "https://api.rootsnsquares.com/innovations/upload-file.php",
        {
          method: "POST",
          body: formData,
        }
      ...
      if (
        ...
        console.error("Rootsnsquares PDF Upload Error Response:", responseData);
        throw new Error(
  ...
  const handleDownloadQRCode = () => {
    ...
    if (!qrCanvas) {
      toast({
        title: "Error",
        description:
          "Could not find QR code to download. Make sure QRCodeCanvas has id='qr-code'.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (qrCodeContentSize <= 0) {
      toast({
        title: "Error",
        description:
          "Download image size is too small for the specified border. Adjust finalImageSize or borderPixels.",
        variant: "destructive",
      });
      return;
    }
  ...
  const generateAndUploadPdf = async (
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          uploadData.message || "Failed to upload PDF for email."
        );
      }
      ...
      toast({
        title: "Error",
        description: `Failed to prepare document for sending: ${
          error.message || error
        } `,
        variant: "destructive",
      });
  ...
  const saveRecord = async () => {
    ...
    if (!customerName || !businessName || invoiceType === "") {
      toast({
        title: "Missing Information",
        description:
          "Customer name, business name, or document type is missing. Cannot save record.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const uploadedPdfUrl = await generateAndUploadPdf(
        invoiceData,
        selectedTemplate,
        invoiceType
      );
      ...
      const invoice_id =
        invoiceType === "invoice"
          ? `INV${Date.now().toString()}${token} `
          : `REC${Date.now().toString()}${token} `;
      ...
      const sendRecordResponse = await fetch(
        "https://api.rootsnsquares.com/innovations/create-records.php",
        {
          method: "POST",
          headers: {
      ...
      if (!sendRecordResponse.ok || sendRecordData.status !== "success") {
        ...
        throw new Error(
          sendRecordData.message || "Failed to save record to database."
        );
      }
      ...
      toast({
        ...
        description:
          sendRecordData.message ||
          `Your ${invoiceType} record has been saved.`,
        variant: "default",
      });
    } catch (error: any) {
      ...
      toast({
        ...
        description: `Failed to save ${invoiceType} record: ${
          error.message || error
        } `,
        variant: "destructive",
      });
  ...
  const handleSendEmailSubmit = async () => {
    ...
    if (!customerName || !businessName || invoiceType === "") {
      toast({
        title: "Missing Information",
        description:
          "Customer name, business name, or document type is missing. Cannot send record.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const sendEmailResponse = await fetch(
        "https://api.rootsnsquares.com/innovations/send-receipt.php",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      toast({
        ...
        description:
          ...
          `Your ${invoiceType} has been sent to ${recipientEmail}.`,
        variant: "default",
      });
      ...
      toast({
        ...
        description: `Failed to send ${invoiceType} to email: ${
          error.message || error
        } `,
        variant: "destructive",
      });
  ...
  const downloadPDF = async () => {
    ...
    // If logged in, proceed with direct download
  ...
  const handleDirectDownload = async () => {
    try {
      ...
      a.href = url;
      a.download = `${
        invoiceType === "invoice" ? "Invoice" : "Receipt"
      }-${businessName.replace(/\s/g, "_")}-${new Date().toISOString()}.pdf`;
          ...
          <div className="relative bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                Sign up or log in to automatically save your {invoiceType}s to
                your profile for easy access later.
              </p>
