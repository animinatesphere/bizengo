...
export const InvoiceGenerator = () => {
  ...
  const [items, setItems] = useState<InvoiceItem[]>([
    { id: "1", description: "", quantity: null, unit_price: null },
  ]);
      ...
      const fetchBusinessData = async () => {
        try {
          const response = await fetch(
            `https://api.bizengo.com/innovations/getverify.php?email=${storedEmail}`
          );
          if (!response.ok) {
          ...
          if (data.status === "success" && data.data && data.data.length > 0) {
            ...
            setBusinessLogo(businessInfo.logo || null);
            setBusinessEmail(businessInfo.email || "");
            setBusinessIdentifier(businessInfo.identifier || "");
            setBusinessIdentifierNumber(businessInfo.number || "");
          } else {
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="text-center">
        ...
        <p className="text-muted-foreground">
          AI-powered invoice creation with QR code generation and smart
          suggestions
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <Select
            value={invoiceType}
            onValueChange={(value) =>
              setInvoiceType(value as InvoiceDocumentType)
            }
          >
            <SelectTrigger>
        ...
        <>
              ...
              {invoiceType === "invoice" && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Only fill this field if you're generating an invoice
                    </CardDescription>
                  </CardHeader>
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <Card className="mb-6">
                    <CardHeader>
                      <CardTitle>
                        {invoiceType === "invoice"
                          ? "Invoice Number & Picture"
                          : "Receipt Number & Picture"}
                      </CardTitle>
                      <CardDescription>
                        {invoiceType === "invoice"
                          ? "Upload a picture (e.g. logo, product, or signature) and enter the invoice number."
                          : "Upload a picture (e.g. product, signature, or stamp) and enter the receipt number."}
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setBusinessIdentifierNumber(e.target.value)
                            }
                          />
                        </div>
                        <div>
                          <Label htmlFor="pictureInput">Picture Upload</Label>
                          <Input
                            id="pictureInput"
                            type="file"
                            accept="image/*"
                ...
                {showPreview && selectedTemplate && (
                  <InvoiceTemplate
                    imageUrl={templateImages[selectedTemplate]}
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-[40%] sm:w-[20%] min-w-[220px]">
                        Description
                      </TableHead>
                      <TableHead className="w-[15%] sm:w-[10%] min-w-[80px]">
                        Quantity
                      </TableHead>
                      <TableHead className="w-[20%] sm:w-[15%] min-w-[160px]">
                        Unit Price (₦)
                      </TableHead>
                      <TableHead className="w-[20%] sm:w-[15%] min-w-[120px]">
                        Price (₦)
                      </TableHead>
                      ...
                      <TableRow key={item.id}>
                        ...
                        <TableCell>
                          <Input
                            type="text"
                            inputMode="numeric"
                            value={formatNumberForDisplay(item.quantity, {
                              maximumFractionDigits: 0,
                            })}
                            onChange={(e) =>
                              handleItemNumberChange(e, item.id, "quantity", 1)
                            }
                            min="1"
                            className="border-0 focus:ring-1 w-full"
                            placeholder="0"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleNumberInputChange(e, setDiscount, 0, 100)
                      }
                      min="0"
                      max="100"
                      step="0.1"
                ...
                <div className="border-t pt-4 space-y-2">
                  ...
                  {(discount ?? 0) > 0 && (
                    <div className="flex justify-between text-green-600">
                      ...
                      {/* Display the original discount for context */}
                  ...
                  {(tax ?? 0) > 0 && (
                    <div className="flex justify-between">
                      <span>Tax ({tax}%):</span>{" "}
                      {/* Display the original tax for context */}
            ...
            <GenerateInvoice
              ...
              businessIdentifierNumber={businessIdentifierNumber}
              ...
              total={total}
              // Pass the uploaded logo for display in the generated invoices/receipt
